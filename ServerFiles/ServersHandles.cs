...
namespace FrostyPipeServer.ServerFiles
{
    ...
    /// <summary>
    /// On receive of a packet an int is read from the start, the packet is sent here to a function bound to that int, providing the connectionid(playerid) and packet, the function called then assumes it knows exactly what the packet contains based on the correct int being read intially. see Server.Run(): packethandlers[i]; for more info
    /// </summary>
    class ServersHandles
    {
        ...
        public static void WelcomeReceived(uint _from, Packet packet)
        {
            try
            {
                // receive
                string name = packet.ReadString();
                string Ridermodel = packet.ReadString();
                string RidermodelBundlename = packet.ReadString();
                string CurrentLevel = "Unknown";
                float VersionNo = 0.00f;
                try
                {
                    CurrentLevel = packet.ReadString();
